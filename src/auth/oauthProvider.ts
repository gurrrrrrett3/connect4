...
export default class DiscordOauthProvider {
    ...
    public async handleCallback(req: Request, res: Response) {
        ...
        let userEntity = await Core.db.em.findOne(User, { id: user.id })
        ...
        if (!userEntity) {
            ...
            await Core.db.em.persistAndFlush(userEntity)
        } else {
    ...
    public static getAvatarUrl(user: User, size: number = 128) {
